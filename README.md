...
.NET разработчик - это fullstack или backend разработчик на языке C#, быстром и простом языке для реализации бизнес-логики enterprise-проектов.
Прежде всего не сайтов-визиток, а полноценных корпоративных приложений. Приложения могут быть как целиком толстым клиентом (WinForms, WPF), либо серверная часть + тонкий клиент (ASP.NET + JS).
...
Для выполнения кода C# понадобится среда разработки (IDE), которая включает в себя компилятор, редактор кода, отладчик, clr и разные фреймвоки (можно выбрать необходимые или добавить потом).
